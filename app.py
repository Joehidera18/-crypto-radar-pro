...
# ============================================================
# CRYPTO RADAR PRO — MARKET MICROSTRUCTURE BUILD — DAY-TRADING RADAR + FOCUS MODE
# ============================================================
#
# Goals:
...
PRELIM_LIMIT = env_int("PRELIM_LIMIT", 60, 20, 120)
TECH_LIMIT = env_int("TECH_LIMIT", 24, 8, 50)
NEWS_LIMIT = env_int("NEWS_LIMIT", 16, 4, 40)
...
def req_json(url, params=None, timeout=3, retries=1, extra_headers=None):
    # Fast-scan guard: no external provider may monopolize a scan.
    timeout = min(float(timeout or 3), 3.0)
    retries = 1
...
def parse_any_feed(url,limit=20):
    try:
        r=session.get(url,timeout=3);r.raise_for_status();root=ET.fromstring(r.content);out=[]
...
def get_coinbase_markets():
    ...
    symbols={};ok=False
    try:
        r=session.get("https://api.exchange.coinbase.com/products",timeout=3);r.raise_for_status();data=r.json()
...
def get_crypto_universe():
    """
    Bootstrap market universe in parallel. A dead/rate-limited provider no longer
    delays trying the fallback provider. First healthy result wins.
    """
    ex=ThreadPoolExecutor(max_workers=2)
    futures={
        ex.submit(get_coingecko_universe):"CoinGecko",
        ex.submit(get_coinpaprika_universe):"CoinPaprika",
    }
    errors=[]
    try:
        done,pending=wait(list(futures),timeout=5)
        # Prefer CoinGecko if both completed successfully.
        ordered=sorted(done,key=lambda f: 0 if futures[f]=="CoinGecko" else 1)
        for f in ordered:
            provider=futures[f]
            try:
                rows=f.result()
                if rows:
                    ...
                    warning=None if provider=="CoinGecko" else "CoinGecko unavailable or slow; using CoinPaprika."
                    return rows,provider,warning
            except Exception as e:
                errors.append(f"{provider}: {e}")
        for p in pending: p.cancel()
        return [],"Unavailable","Market providers timed out. "+("; ".join(errors) if errors else "")
    finally:
        ex.shutdown(wait=False,cancel_futures=True)
...
def get_binance_us_symbols():
    try:
        j = cached_req_json(f"{BINANCE_US}/api/v3/exchangeInfo", timeout=3, retries=1, ttl=900)
        return {
...
# ---------------------- MICROSTRUCTURE / LIQUIDITY INTELLIGENCE ----------------------

BINANCE_FUTURES = "https://fapi.binance.com"

def liquidity_map_from_raw(raw, lookback=100):
    """
    Maps nearby pools where stops/liquidity commonly cluster:
    repeated swing highs/lows, equal highs/lows and recent extremes.
    This is a heuristic map, not exchange liquidation data.
    """
    if not raw or len(raw) < 35:
        return {"above":[],"below":[],"nearest_above":None,"nearest_below":None,"score":50}
    highs=[sf(r[2]) for r in raw[-lookback:]]
    lows=[sf(r[3]) for r in raw[-lookback:]]
    closes=[sf(r[4]) for r in raw[-lookback:]]
    price=closes[-1]
    atr=max(atr_value(highs,lows,closes,14),price*0.001)
    tol=max(atr*.30,price*.0012)
    ...
    def cluster(points, side):
        ...
        for _,px in points:
            found=None
            for c in clusters:
                if abs(c["price"]-px) <= tol:
                    found=c;break
            if found:
                found["prices"].append(px);found["touches"]+=1;found["price"]=mean(found["prices"])
            else:
                clusters.append({"price":px,"prices":[px],"touches":1})
        out=[]
        for c in clusters:
            px=c["price"]
            if (side=="above" and px>price) or (side=="below" and px<price):
                distance=abs(px-price)/price*100
                strength=clamp(35+c["touches"]*15-max(distance-1,0)*2)
                out.append({"price":round(px,8),"distance_pct":round(distance,2),"touches":c["touches"],"strength":round(strength,1)})
        out.sort(key=lambda z:(z["distance_pct"],-z["strength"]))
        return out[:5]

    above=cluster(sh[-24:],"above")
    below=cluster(sl[-24:],"below")
    na=above[0] if above else None
    nb=below[0] if below else None

    # A nearby pool above with room from support is constructive for a long;
    # a huge nearby sell-side pool immediately overhead is less attractive.
    score=50
    if na and .4 <= na["distance_pct"] <= 4: score+=10
    if nb and nb["distance_pct"] <= 1.2: score+=8
    if na and na["distance_pct"] < .35 and na["strength"] >= 65: score-=12
    return {"above":above,"below":below,"nearest_above":na,"nearest_below":nb,"score":round(clamp(score),1)}

def absorption_proxy(raw, lookback=12):
    """Candle/volume proxy for absorption: large activity with limited progress + rejection."""
    if not raw or len(raw)<25:
        return {"score":50,"label":"neutral","evidence":[]}
    r=raw[-lookback:]
    opens=[sf(x[1]) for x in r]; highs=[sf(x[2]) for x in r]; lows=[sf(x[3]) for x in r]
    closes=[sf(x[4]) for x in r]; qv=[sf(x[7]) for x in r]
    base=[sf(x[7]) for x in raw[-45:-lookback]] or qv
    basev=max(mean(base),1)
    bull=bear=0;evidence=[]
    for o,h,l,c,v in zip(opens,highs,lows,closes,qv):
        rng=max(h-l,1e-12); body=abs(c-o); vr=v/basev
        lower=min(o,c)-l; upper=h-max(o,c)
        if vr>=1.4 and body/rng<.45:
            if lower/rng>.35:
                bull+=vr; evidence.append("heavy volume absorbed near candle lows")
            if upper/rng>.35:
                bear+=vr; evidence.append("heavy volume absorbed near candle highs")
    delta=bull-bear
    score=clamp(50+delta*8)
    label="buyer absorption" if score>=62 else "seller absorption" if score<=38 else "neutral"
    return {"score":round(score,1),"label":label,"evidence":list(dict.fromkeys(evidence))[:3]}

def flow_toxicity_proxy(raw, spread_bps=None, depth=None, lookback=50):
    """
    VPIN-inspired proxy using directional quote-volume imbalance, volatility and spread.
    It is intentionally labeled a proxy rather than true VPIN.
    """
    ...
    prev=None
    for x in r:
        o,c,q=sf(x[1]),sf(x[4]),sf(x[7])
        sign=1 if c>o else -1 if c<o else 0
        signed += sign*q; total += q
        if prev and prev>0: rets.append(abs(c/prev-1)*100)
        prev=c
    imbalance=abs(safe_div(signed,total,0))*100
    vol=mean(rets[-20:],0)
    score=35 + imbalance*.55 + min(vol*18,25)
    if spread_bps is not None: score += min(max(spread_bps-3,0)*1.2,18)
    if depth is not None and depth<100_000: score+=10
    score=round(clamp(score),1)
    label="high toxicity" if score>=68 else "elevated" if score>=55 else "healthy"
    return {"score":score,"label":label,"directional_imbalance_pct":round(imbalance,1)}

def orderbook_microstructure(bids, asks, mid):
    if not bids or not asks or mid<=0:
        return {"score":50,"bid_wall":None,"ask_wall":None,"pressure":"unknown"}
    def wall(rows, side):
        candidates=[]
        for px,q in rows:
            dist=abs(px-mid)/mid*100
            if dist<=1.5:
                candidates.append((px*q,px,dist))
        if not candidates:return None
        notion,px,dist=max(candidates)
        return {"price":round(px,8),"notional":round(notion,2),"distance_pct":round(dist,2)}
    bw=wall(bids,"bid"); aw=wall(asks,"ask")
    bd=sum(px*q for px,q in bids if px>=mid*.9975)
    ad=sum(px*q for px,q in asks if px<=mid*1.0025)
    im=safe_div(bd-ad,bd+ad,0)
    score=clamp(50+im*45)
    if bw and (not aw or bw["notional"]>aw["notional"]*1.4): score+=8
    if aw and (not bw or aw["notional"]>bw["notional"]*1.4): score-=8
    pressure="buyers" if score>=60 else "sellers" if score<=40 else "balanced"
    return {"score":round(clamp(score),1),"bid_wall":bw,"ask_wall":aw,"pressure":pressure}

def setup_state_machine(technical, prebreakout_score, entry_quality, chase):
    """Turns static indicators into a readable setup lifecycle."""
    if not technical.get("available"):
        return {"stage":"DATA LIMITED","progress":10,"next":"wait for technical data"}
    ...
    post=sf(technical.get("post_breakout_score"),50)
    fake=sf(technical.get("fakeout_score"),50)
    micro=sf(technical.get("microstructure_score"),50)
    if chase>=18 or str(technical.get("market_phase"))=="EXHAUSTION RISK":
        return {"stage":"EXTENDED","progress":92,"next":"wait for reset / retest"}
    if fake<=35:
        return {"stage":"INVALIDATION RISK","progress":25,"next":"avoid until structure repairs"}
    if post>=70 and breakout>=65:
        return {"stage":"CONFIRMED","progress":88,"next":"look for controlled retest / entry trigger"}
    if sweep>=68 and entry_quality>=55:
        return {"stage":"SWEEP / RECLAIM","progress":76,"next":"confirm reclaim with 5m/15m volume and order flow"}
    if breakout>=62 and micro>=55:
        return {"stage":"BREAKOUT ATTEMPT","progress":70,"next":"wait for close + retest hold"}
    if prebreakout_score>=70 and squeeze>=65:
        return {"stage":"COMPRESSION","progress":58,"next":"wait for expansion through mapped liquidity"}
    if entry_quality>=60:
        return {"stage":"BUILDING","progress":42,"next":"wait for momentum / volume confirmation"}
    return {"stage":"WATCHING","progress":25,"next":"no entry yet"}

def derivatives_context(asset):
    """
    Optional public Binance futures context. Failure is neutral and never blocks the core radar.
    Uses funding, futures basis and open-interest change where that contract exists.
    """
    symbol=f"{str(asset.get('symbol','')).upper()}USDT"
    neutral={"available":False,"score":50,"funding_rate":None,"basis_pct":None,"oi_change_pct":None,"label":"unavailable"}
    try:
        premium=cached_req_json(f"{BINANCE_FUTURES}/fapi/v1/premiumIndex",{"symbol":symbol},timeout=2,retries=1,ttl=45)
        hist=cached_req_json(f"{BINANCE_FUTURES}/futures/data/openInterestHist",{"symbol":symbol,"period":"5m","limit":12},timeout=2,retries=1,ttl=60)
        funding=sf(premium.get("lastFundingRate"))*100
        mark=sf(premium.get("markPrice")); idx=sf(premium.get("indexPrice"))
        basis=safe_div(mark-idx,idx,0)*100 if idx else 0
        oi_change=None
        if isinstance(hist,list) and len(hist)>=2:
            first=sf(hist[0].get("sumOpenInterestValue"))
            last=sf(hist[-1].get("sumOpenInterestValue"))
            oi_change=safe_div(last-first,first,0)*100 if first else None
        score=50
        label="balanced"
        # Crowded long positioning is a caution, not automatically bearish.
        if funding>0.05: score-=12;label="crowded longs"
        elif funding<-0.03: score+=7;label="short-heavy / squeeze potential"
        if oi_change is not None:
            if oi_change>5 and funding>0.03: score-=8;label="leveraged long build"
            elif oi_change>3 and abs(funding)<0.03: score+=6;label="healthy participation"
            elif oi_change<-5: score-=3
        if abs(basis)>.35: score-=5
        return {"available":True,"score":round(clamp(score),1),"funding_rate":round(funding,4),
                "basis_pct":round(basis,3),"oi_change_pct":round(oi_change,2) if oi_change is not None else None,
                "label":label}
    except Exception:
        return neutral

def execution_style(asset, technical):
    liq=sf(asset.get("liquidity_score"),50)
    tox=sf(technical.get("toxicity_score"),50)
    chase=sf(technical.get("chase_proxy"),0)
    if liq>=68 and tox<=55:
        return "momentum / continuation"
    if liq<42 or tox>=68:
        return "mean-reversion caution"
    return "balanced"
...
def technical_for_asset(asset, available_symbols, horizon="4h"):
    ...
    if pair not in available_symbols:
        return {
            ...
            "liquidity_map":{"above":[],"below":[],"nearest_above":None,"nearest_below":None,"score":50},
            "absorption":{"score":50,"label":"neutral","evidence":[]},"toxicity_score":50.0,"toxicity_label":"unknown",
            "microstructure_score":50.0,"orderbook_pressure":"unknown","bid_wall":None,"ask_wall":None,
            "early_acceleration_score":50.0,"early_acceleration_reasons":[]
        }

    try:
        frames={}
        raw1m=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"1m","limit":220},timeout=3,retries=1,ttl=20)
        raw5m=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"5m","limit":220},timeout=3,retries=1,ttl=45)
        raw15m=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"15m","limit":220},timeout=3,retries=1,ttl=90)
        raw1h=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"1h","limit":220},timeout=3,retries=1,ttl=240)
        raw4h=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"4h","limit":220},timeout=3,retries=1,ttl=600)
        # Preserve higher-timeframe context; long candles are cached aggressively.
        raw1d=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"1d","limit":365},timeout=3,retries=1,ttl=1800)
        raw1w=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"1w","limit":180},timeout=3,retries=1,ttl=7200)
        raw1M=cached_req_json(f"{BINANCE_US}/api/v3/klines",{"symbol":pair,"interval":"1M","limit":60},timeout=3,retries=1,ttl=21600)
        ...
        liquidity_map = liquidity_map_from_raw(active_raw,100)
        absorption = absorption_proxy(active_raw,12)
        ...
        book=req_json(f"{BINANCE_US}/api/v3/depth",{"symbol":pair,"limit":100},timeout=3,retries=1)
        ...
        micro_book={"score":50,"bid_wall":None,"ask_wall":None,"pressure":"unknown"}
        if bids and asks:
            ...
            micro_book=orderbook_microstructure(bids,asks,mid)

        toxicity=flow_toxicity_proxy(active_raw,spread_bps,depth,50)
        microstructure_score=clamp(
            orderbook_score*.38 +
            sf(micro_book.get("score"),50)*.24 +
            sf(cvd.get("score"),50)*.18 +
            sf(absorption.get("score"),50)*.12 +
            sf(liquidity_map.get("score"),50)*.08
        )
        ...
        multi=clamp(
            ...
            post_breakout*0.11 +
            fakeout*0.05 +
            cvd.get("score",50)*0.05 +
            active.get("divergence_score",50)*0.03 +
            analog.get("score",50)*0.04 +
            microstructure_score*0.10 +
            (100-sf(toxicity.get("score"),50))*0.04
        )
        ...
        if absorption.get("label")!="neutral": signals.append(absorption.get("label"))
        if micro_book.get("pressure")!="unknown": signals.append(f"order-book pressure: {micro_book.get('pressure')}")
        if toxicity.get("label") in ("elevated","high toxicity"): signals.append(f"flow toxicity: {toxicity.get('label')}")
        if liquidity_map.get("nearest_above"): signals.append(f"liquidity pool {liquidity_map['nearest_above']['distance_pct']}% above")
        ...
        return {
            ...
            "toxicity_score":toxicity.get("score",50),
            "toxicity_label":toxicity.get("label","unknown"),
            "microstructure_score":round(microstructure_score,1),
            "orderbook_pressure":micro_book.get("pressure","unknown"),
            "bid_wall":micro_book.get("bid_wall"),
            "ask_wall":micro_book.get("ask_wall"),
...
def init_prediction_db():
    try:
        with prediction_db_lock:
            ...
            """)
            try:
                con.execute("ALTER TABLE predictions ADD COLUMN features_json TEXT")
            except Exception:
                pass
            con.commit()
            con.close()
        ...
        with prediction_db_lock:
            ...
            for r in rows[:10]:
                if r.get("action") not in ("SETUP READY","WATCH"):
                    continue
                feature_snapshot={
                    "regime":r.get("market_alignment"),"setup":r.get("setup_type"),
                    "lifecycle":r.get("setup_lifecycle"),"microstructure":r.get("microstructure_score"),
                    "toxicity":r.get("toxicity_score"),"liquidity_map":r.get("liquidity_map"),
                    "derivatives":r.get("derivatives"),"news":r.get("news_intelligence"),
                    "entry_quality":r.get("entry_quality"),"prebreakout":r.get("prebreakout_score"),
                    "confirmation":r.get("confirmation_quality"),"rsi":r.get("rsi"),
                    "spread_bps":r.get("spread_bps"),"book_imbalance":r.get("book_imbalance"),
                }
                con.execute("""
                    INSERT OR IGNORE INTO predictions
                    (bucket,created_at,updated_at,symbol,horizon,setup_type,entry,stop,target1,target2,score,do_not_trade,max_seen,min_seen,status,features_json)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'OPEN',?)
                """,(
                    ...
                    json.dumps(feature_snapshot,separators=(",",":"),default=str)
                ))
            con.commit();con.close()
    except Exception:
        pass
...
# ---------------------- V16 EFFICIENT DATA LAYER ----------------------
# Reuse identical provider responses instead of repeatedly downloading the same
# market/reference data during one or many scans.
...
def provider_ttl(url, params=None):
    u=str(url).lower()
    p=dict(params or {})
    interval=str(p.get("interval",""))
    # Live-ish endpoints stay fresh; slower structural/reference feeds live longer.
    if "ticker/bookticker" in u or "ticker/24hr" in u:
        return 8
    if "klines" in u:
        return {"1m":20,"5m":45,"10m":60,"15m":90,"1h":240,"4h":600,"1d":1800,"1w":7200,"1M":21600}.get(interval,120)
    if "exchangeinfo" in u or "products" in u:
        return 900
    if "fear" in u or "alternative.me" in u:
        return 600
    if "reddit" in u or "news" in u:
        return 180
    return 45

def cached_req_json(url, params=None, timeout=3, retries=1, extra_headers=None, ttl=None):
    key=(str(url),_freeze_params(params))
    now=time.time()
    with provider_cache_lock:
        hit=provider_cache.get(key)
        if hit and now-hit[0] < (ttl if ttl is not None else provider_ttl(url,params)):
            return hit[1]
        evt=provider_inflight.get(key)
        if evt is None:
            evt=threading.Event()
            provider_inflight[key]=evt
            owner=True
        else:
            owner=False

    if not owner:
        # Another worker is already fetching this exact resource. Reuse it.
        evt.wait(timeout=min(float(timeout or 3)+1,5))
        with provider_cache_lock:
            hit=provider_cache.get(key)
            if hit:
                return hit[1]
    ...
    try:
        data=req_json(url,params=params,timeout=timeout,retries=retries,extra_headers=extra_headers)
        with provider_cache_lock:
            provider_cache[key]=(time.time(),data)
        return data
    finally:
        with provider_cache_lock:
            e=provider_inflight.pop(key,None)
            if e: e.set()

def cache_stats():
    now=time.time()
    with provider_cache_lock:
        live=sum(1 for _,(ts,_) in provider_cache.items() if now-ts < 3600)
        return {"entries":len(provider_cache),"recent_entries":live,"inflight":len(provider_inflight)}
...
MIN_CALIBRATION_SAMPLES = env_int("MIN_CALIBRATION_SAMPLES", 30, 10, 500)

def performance_breakdown(horizon=None, setup_type=None, regime_label=None, min_samples=MIN_CALIBRATION_SAMPLES):
    try:
        with prediction_db_lock:
            con=sqlite3.connect(PREDICTION_DB,timeout=5)
            q="SELECT outcome,features_json FROM predictions WHERE status='CLOSED'"
            args=[]
            if horizon:
                q+=" AND horizon=?"; args.append(horizon)
            if setup_type:
            ...
            con.close()
        filtered=[]
        for outcome,features_json in rows:
            features={}
            if features_json:
                try: features=json.loads(features_json)
                except Exception: features={}
            if regime_label and str(features.get("regime") or "").upper()!=str(regime_label).upper():
                continue
            filtered.append((outcome,features))
        samples=len(filtered)
        wins=sum(1 for o,_ in filtered if o in ("TARGET1","TARGET2"))
        target2=sum(1 for o,_ in filtered if o=="TARGET2")
        losses=sum(1 for o,_ in filtered if o=="STOP")
        return {
            "samples":samples,"wins":wins,"losses":losses,"target2_hits":target2,
            "win_rate":round(wins/samples*100,1) if samples else None,
            "target2_rate":round(target2/samples*100,1) if samples else None,
            "calibrated":samples>=min_samples
        }
    except Exception:
        return {"samples":0,"wins":0,"losses":0,"target2_hits":0,"win_rate":None,"target2_rate":None,"calibrated":False}

def evidence_weighted_score(row, horizon, regime_label):
    base=sf(row.get("score"),50)
    setup=row.get("setup_type")
    overall=performance_breakdown(horizon,setup,None)
    regime_perf=performance_breakdown(horizon,setup,regime_label)
    chosen=regime_perf if regime_perf.get("calibrated") else overall if overall.get("calibrated") else None
    if not chosen:
        return round(base,1), {
            "calibrated":False,
            "samples":max(overall.get("samples",0),regime_perf.get("samples",0)),
            "win_rate":None,"label":"collecting evidence"
        }
    wr=sf(chosen.get("win_rate"),50)
    evidence_component=clamp((wr-50)*1.15+50)
    blended=base*.78+evidence_component*.22
    return round(clamp(blended),1), {
        "calibrated":True,"samples":chosen.get("samples",0),
        "win_rate":chosen.get("win_rate"),"target2_rate":chosen.get("target2_rate"),
        "label":"regime-matched" if chosen is regime_perf else "setup history"
    }

def setup_quality_gate(row):
    reasons=[]
    if sf(row.get("do_not_trade_score"),50)>=62: reasons.append("do-not-trade risk too high")
    if sf(row.get("confirmation_quality"),50)<52: reasons.append("confirmation is weak")
    if sf(row.get("signal_disagreement"),50)>=68: reasons.append("signals conflict")
    if sf(row.get("risk_reward"),0)<1.5: reasons.append("risk/reward below minimum")
    if sf(row.get("toxicity_score"),50)>=72: reasons.append("flow toxicity is high")
    if str(row.get("market_alignment") or "").lower()=="hostile": reasons.append("broad market is hostile")
    if sf(row.get("entry_quality"),50)<45: reasons.append("entry location is poor")
    if sf(row.get("chase_penalty"),0)>=18: reasons.append("price is extended")
    return len(reasons)==0 and row.get("action")!="AVOID", reasons[:6]

def confluence_groups(row):
    technical=mean([sf(row.get("technical_score"),50),sf(row.get("prebreakout_score"),50),sf(row.get("entry_quality"),50)],50)
    micro=mean([sf(row.get("microstructure_score"),50),100-sf(row.get("toxicity_score"),50),sf((row.get("absorption") or {}).get("score"),50)],50)
    context=mean([sf(row.get("market_alignment_score"),50),sf(row.get("relative_strength"),50)],50)
    info=mean([sf(row.get("news_score"),50),sf(row.get("social_score"),50),sf((row.get("derivatives") or {}).get("score"),50)],50)
    return {"technical":round(clamp(technical),1),"microstructure":round(clamp(micro),1),
            "market_context":round(clamp(context),1),"information":round(clamp(info),1)}
...
# ---------------------- V16 HARD SCAN DEADLINES ----------------------
SCAN_BUDGET_SECONDS = env_int("SCAN_BUDGET_SECONDS", 25, 15, 40)
TECH_STAGE_SECONDS = env_int("TECH_STAGE_SECONDS", 14, 8, 25)
NEWS_STAGE_SECONDS = env_int("NEWS_STAGE_SECONDS", 6, 3, 12)

def run_stage_with_deadline(items, worker, id_key, seconds, max_workers=8):
    """
    Run a rich analysis stage without allowing one slow provider to hold the
    entire HTTP request open. Completed work is returned; unfinished work is
    cancelled where possible and represented by normal fallback values.
    """
    if not items:
        return {}, 0, 0
    ex=ThreadPoolExecutor(max_workers=max_workers)
    futs={ex.submit(worker,item):item[id_key] for item in items}
    done, pending = wait(list(futs.keys()), timeout=max(0.25,float(seconds)))
    out={}
    for f in done:
        ident=futs[f]
        try:
    ...
    for f in pending:
        f.cancel()
    # Crucial: do not make the web request wait for unfinished optional work.
    ex.shutdown(wait=False, cancel_futures=True)
...
def enhanced_market_regime(rows, fng_value):
    """Broad regime from BTC/ETH trend, market breadth, BTC dominance and ETH/BTC rotation."""
    btc=next((x for x in rows if str(x.get("symbol","")).lower()=="btc"),{})
    eth=next((x for x in rows if str(x.get("symbol","")).lower()=="eth"),{})
    def ch(x,k): return sf(x.get(k))
    b24=ch(btc,"price_change_percentage_24h_in_currency"); b7=ch(btc,"price_change_percentage_7d_in_currency")
    e24=ch(eth,"price_change_percentage_24h_in_currency"); e7=ch(eth,"price_change_percentage_7d_in_currency")
    stable_symbols = globals().get("STABLE_SYMBOLS", set())
    sample=[x for x in rows[:100] if str(x.get("symbol","")).upper() not in stable_symbols]
    breadth24=100*sum(ch(x,"price_change_percentage_24h_in_currency")>0 for x in sample)/max(1,len(sample))
    breadth7=100*sum(ch(x,"price_change_percentage_7d_in_currency")>0 for x in sample)/max(1,len(sample))
    breadth=.45*breadth24+.55*breadth7
    total=sum(max(0,sf(x.get("market_cap"))) for x in rows)
    dom=100*sf(btc.get("market_cap"))/total if total else 50
    rotation=clamp(50+(e24-b24)*3+(e7-b7)*2)
    trend=clamp(50+b24*2+b7*1.4+e24*1.2+e7*.8)
    sentiment=clamp(fng_value)
    score=round(clamp(.40*trend+.25*breadth+.20*rotation+.15*sentiment),1)
    alt=round(clamp(.50*breadth+.35*rotation+.15*clamp(100-(dom-35)*2.2)),1)
    label="STRONG BULL" if score>=72 else "BULL" if score>=60 else "NEUTRAL / CHOP" if score>=44 else "BEAR" if score>=30 else "STRONG BEAR"
    alt_label="FAVORABLE" if alt>=65 else "NEUTRAL" if alt>=43 else "HOSTILE"
    return {"label":label,"score":score,"alt_environment":alt_label,"alt_score":alt,
            "breadth":round(breadth,1),"btc_dominance":round(dom,2),
            "ethbtc_strength":round(rotation,1),"trend_score":round(trend,1),
...
def scan_crypto(horizon, risk):
    ...
    regime = enhanced_market_regime(rows, fng_value)
    ...
    # Deep layers are rich, but they have strict stage budgets. Slow optional
    # providers can no longer turn a 25-second scan into a 10-minute request.
    scan_started=time.monotonic()
    tech_assets = prelim[:TECH_LIMIT]
    technical_map, tech_done, tech_pending = run_stage_with_deadline(
        tech_assets,
        lambda a: technical_for_asset(a, available_symbols, horizon),
        "id",
        min(TECH_STAGE_SECONDS, max(2, SCAN_BUDGET_SECONDS-5)),
        max_workers=8
    )

    elapsed=time.monotonic()-scan_started
    remaining=max(2, SCAN_BUDGET_SECONDS-elapsed-3)
    news_assets = prelim[:NEWS_LIMIT]
    deriv_assets = prelim[:min(12,len(prelim))]
    # Optional information layers run side-by-side so richer data does not add serial latency.
    optional_ex=ThreadPoolExecutor(max_workers=2)
    f_news=optional_ex.submit(
        run_stage_with_deadline,news_assets,lambda a:catalyst_for_asset(a,horizon),"id",
        min(NEWS_STAGE_SECONDS,remaining),8
    )
    f_deriv=optional_ex.submit(
        run_stage_with_deadline,deriv_assets,derivatives_context,"id",
        min(5,remaining),8
    )
    catalyst_map,news_done,news_pending=f_news.result()
    derivatives_map,deriv_done,deriv_pending=f_deriv.result()
    optional_ex.shutdown(wait=False,cancel_futures=True)
    ...
    for a in prelim:
        ...
        deriv = derivatives_map.get(a["id"],{"available":False,"score":50,"funding_rate":None,"basis_pct":None,"oi_change_pct":None,"label":"unavailable"})
        ...
        micro=sf(technical.get("microstructure_score"),50)
        tox=sf(technical.get("toxicity_score"),50)
        deriv_score=sf(deriv.get("score"),50)
        micro_w={"1h":0.10,"4h":0.09,"12h":0.07,"24h":0.05}.get(horizon,0.03)
        score=clamp(score*(1-micro_w)+micro*micro_w-(max(tox-65,0)*0.08))
        deriv_w={"1h":0.05,"4h":0.06,"12h":0.06,"24h":0.05}.get(horizon,0.025)
        score=clamp(score*(1-deriv_w)+deriv_score*deriv_w)
        ...
        lifecycle = setup_state_machine(technical,prebreakout_score,entry_quality,chase)
        ...
        row = {
            **a,
            ...
            "derivatives":deriv,
            "liquidity_map":technical.get("liquidity_map",{}),
            "microstructure_score":technical.get("microstructure_score",50),
            "toxicity_score":technical.get("toxicity_score",50),
            "toxicity_label":technical.get("toxicity_label","unknown"),
            "absorption":technical.get("absorption",{}),
            "orderbook_pressure":technical.get("orderbook_pressure","unknown"),
            "bid_wall":technical.get("bid_wall"),
            "ask_wall":technical.get("ask_wall"),
            "execution_style":execution_style(a,technical),
    ...
    for r in final:
        r["confluence_groups"]=confluence_groups(r)
        r["heuristic_score"]=r.get("score")
        r["score"],r["evidence"]=evidence_weighted_score(r,horizon,regime.get("label"))
        actionable,gate_reasons=setup_quality_gate(r)
        r["actionable"]=actionable
        r["quality_gate_reasons"]=gate_reasons
        if not actionable:
            r["action"]="AVOID" if len(gate_reasons)>=3 else "WAIT"

    final.sort(key=lambda z:(z["actionable"],z["score"],z["confidence"]), reverse=True)
    ...
    return {
        ...
        "diagnostics":{
            ...
            "technical_requested":len(tech_assets),
            "technical_completed":tech_done,
            "technical_timed_out":tech_pending,
            "news_requested":len(news_assets),
            "news_completed":news_done,
            "news_timed_out":news_pending,
            "derivatives_requested":len(deriv_assets),
            "derivatives_completed":deriv_done,
            "derivatives_timed_out":deriv_pending,
            "scan_budget_seconds":SCAN_BUDGET_SECONDS,"route_hard_stop_seconds":29,
        }
    }
...
def get_scan(asset,horizon,risk,force=False):
    ...
    acquired=scan_compute_lock.acquire(timeout=2)
    if not acquired:
        with cache_lock:
            cached=cache.get(key)
            if cached:
                stale=dict(cached)
                stale["warning"]="A previous scan is still finishing; showing the latest cached ranking instead of blocking."
                return stale
        return {
            "updated":int(time.time()),"provider":"busy","warning":"A previous scan is still finishing. Retry in a few seconds.",
            "regime":{"label":"BUSY","score":50},"fear_greed":{"value":50,"label":"Unavailable"},
            "diagnostics":{"scan_busy":True},"results":[]
        }
    try:
        with cache_lock:
            cached = cache.get(key)
...
def api_scan():
    ...
    # Absolute route deadline: never leave the browser spinning for minutes.
    q=queue.Queue(maxsize=1)
    def _work():
        try:
            q.put(("ok",get_scan(asset,horizon,risk,force)),block=False)
        except Exception as e:
            try:q.put(("err",str(e)),block=False)
            except Exception:pass
    th=threading.Thread(target=_work,daemon=True)
    th.start()
    try:
        kind,payload=q.get(timeout=29)
    except queue.Empty:
        return jsonify({
            "updated":int(time.time()),
            "provider":"partial / timeout",
            "warning":"The server stopped this scan at 29 seconds instead of hanging. Tap Refresh Rankings to retry; cached data from completed providers is kept.",
            "regime":{"label":"UNKNOWN","score":50},
            "fear_greed":{"value":50,"label":"Unavailable"},
            "diagnostics":{"hard_timeout":True,"deadline_seconds":29},
            "results":[]
        }),200
    if kind=="err":
        return jsonify({"updated":int(time.time()),"error":payload,"results":[]}),500
...
def health():
    return {
        "ok":True,
        "version":"17.0-microstructure",
        "stocks_enabled":finnhub_enabled(),"scan_budget_seconds":SCAN_BUDGET_SECONDS,"cache":cache_stats()
    }
...
# ---------------------- BUILT-IN PAPER TRADER ----------------------

PAPER_DB_PATH = os.environ.get("PAPER_DB_PATH", "/tmp/crypto_radar_paper.sqlite3")
PAPER_STARTING_BALANCE = float(os.environ.get("PAPER_STARTING_BALANCE", "500"))
PAPER_RISK_PER_TRADE = float(os.environ.get("PAPER_RISK_PER_TRADE", "0.01"))
PAPER_MAX_OPEN_TRADES = env_int("PAPER_MAX_OPEN_TRADES", 3, 1, 20)
PAPER_MAX_POSITION_FRACTION = float(os.environ.get("PAPER_MAX_POSITION_FRACTION", "0.25"))
PAPER_FEE_RATE = float(os.environ.get("PAPER_FEE_RATE", "0.004"))
PAPER_SLIPPAGE_RATE = float(os.environ.get("PAPER_SLIPPAGE_RATE", "0.001"))
PAPER_INTERVAL_SECONDS = env_int("PAPER_INTERVAL_SECONDS", 300, 60, 3600)
PAPER_ENABLED = str(os.environ.get("PAPER_ENABLED", "1")).lower() not in ("0","false","no","off")
PAPER_HORIZON = os.environ.get("PAPER_HORIZON", "4h")
PAPER_RISK_MODE = os.environ.get("PAPER_RISK_MODE", "aggressive")
PAPER_MAX_TRADE_AGE_HOURS = float(os.environ.get("PAPER_MAX_TRADE_AGE_HOURS", "24"))

paper_lock = threading.Lock()
paper_thread_started = False

def init_paper_db():
    try:
        os.makedirs(os.path.dirname(PAPER_DB_PATH) or ".", exist_ok=True)
        with paper_lock:
            con=sqlite3.connect(PAPER_DB_PATH,timeout=5)
            con.executescript("""
            ...
            """)
            if not con.execute("SELECT 1 FROM paper_account WHERE id=1").fetchone():
                con.execute("INSERT INTO paper_account(id,starting_balance,cash,realized_pnl,fees_paid,updated_at) VALUES(1,?,?,?,?,?)",
                            (PAPER_STARTING_BALANCE,PAPER_STARTING_BALANCE,0.0,0.0,int(time.time())))
            con.commit();con.close()
    except Exception as e:
        print("paper db init:",e)
...
def paper_fee(notional):
    return max(0.0,sf(notional))*PAPER_FEE_RATE

def paper_buy_fill(price):
    return sf(price)*(1+PAPER_SLIPPAGE_RATE)

def paper_sell_fill(price):
    return sf(price)*(1-PAPER_SLIPPAGE_RATE)

def paper_open_rows(con):
    return list(con.execute("SELECT * FROM paper_trades WHERE status='OPEN' ORDER BY opened_at"))

def paper_quote(signal):
    pair=str(signal.get("live_pair") or "").upper()
    if pair:
        try:
            j=cached_req_json(f"{BINANCE_US}/api/v3/ticker/price",{"symbol":pair},timeout=2,retries=1,ttl=2)
            px=sf(j.get("price"))
            if px>0:return px
        except Exception: pass
    return sf(signal.get("price"))

def paper_signal_from_row(row):
    try:return json.loads(row["signal_json"])
    except Exception:return {"symbol":row["symbol"],"live_pair":row["live_pair"]}

def paper_equity_value(con):
    acc=con.execute("SELECT * FROM paper_account WHERE id=1").fetchone()
    open_value=0.0
    for tr in paper_open_rows(con):
        px=paper_quote(paper_signal_from_row(tr))
        if px<=0:px=sf(tr["entry_fill"])
        open_value += sf(tr["qty_open"])*px
    return sf(acc["cash"])+open_value,open_value

def paper_eligible(s):
    if not s.get("actionable",False):return False
    if str(s.get("action") or "").upper() not in ("SETUP READY","WATCH"):return False
    if not s.get("coinbase_available",False):return False
    lo,hi,stop,t1,t2=map(sf,[s.get("entry_low"),s.get("entry_high"),s.get("stop"),s.get("target1"),s.get("target2")])
    if min(lo,hi,stop,t1,t2)<=0:return False
    if stop>=max(lo,hi) or t1<=max(lo,hi):return False
    return True

def paper_in_entry_zone(price,s):
    lo,hi=sorted((sf(s.get("entry_low")),sf(s.get("entry_high"))))
    tol=price*.0025
    return lo-tol<=price<=hi+tol

def paper_position_size(equity,cash,entry,stop):
    dist=entry-stop
    if dist<=0:return 0.0,0.0
    risk_budget=equity*PAPER_RISK_PER_TRADE
    qty_risk=risk_budget/dist
    notional_cap=min(cash*.98,equity*PAPER_MAX_POSITION_FRACTION)
    qty_cash=notional_cap/(entry*(1+PAPER_FEE_RATE+PAPER_SLIPPAGE_RATE))
    qty=max(0,min(qty_risk,qty_cash))
    return qty,qty*dist

def paper_open_trade(con,s,market_price):
    acc=con.execute("SELECT * FROM paper_account WHERE id=1").fetchone()
    equity,_=paper_equity_value(con)
    fill=paper_buy_fill(market_price);stop=sf(s.get("stop"))
    qty,risk_dollars=paper_position_size(equity,sf(acc["cash"]),fill,stop)
    if qty<=0:return None
    notional=qty*fill;fee=paper_fee(notional)
    if notional+fee>sf(acc["cash"]):return None
    cur=con.execute("""INSERT INTO paper_trades(
        symbol,name,live_pair,setup_type,regime,horizon,risk_mode,opened_at,status,
        entry_signal,entry_fill,stop_initial,stop_current,target1,target2,qty_initial,qty_open,
        position_notional,risk_dollars,entry_fee,max_price,min_price,signal_score,evidence_json,signal_json
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(
        str(s.get("symbol") or "").upper(),s.get("name"),s.get("live_pair"),s.get("setup_type"),
        s.get("market_alignment"),PAPER_HORIZON,PAPER_RISK_MODE,int(time.time()),"OPEN",
        market_price,fill,stop,stop,sf(s.get("target1")),sf(s.get("target2")),qty,qty,notional,
        risk_dollars,fee,fill,fill,sf(s.get("score")),
        json.dumps(s.get("evidence") or {},separators=(",",":"),default=str),
        json.dumps(s,separators=(",",":"),default=str)
    ))
    con.execute("UPDATE paper_account SET cash=cash-?,fees_paid=fees_paid+?,updated_at=? WHERE id=1",
                (notional+fee,fee,int(time.time())))
    con.commit()
    return cur.lastrowid

def paper_realize(con,tr,qty,trigger,reason,close_trade):
    qty=min(sf(qty),sf(tr["qty_open"]))
    if qty<=0:return
    fill=paper_sell_fill(trigger);proceeds=qty*fill;fee=paper_fee(proceeds)
    pnl=qty*(fill-sf(tr["entry_fill"]))-fee
    new_qty=max(0,sf(tr["qty_open"])-qty)
    realized=sf(tr["realized_pnl"])+pnl
    status="CLOSED" if close_trade or new_qty<=1e-12 else "OPEN"
    total_trade_pnl=realized-sf(tr["entry_fee"])
    rmult=total_trade_pnl/max(sf(tr["risk_dollars"]),1e-9) if status=="CLOSED" else None
    con.execute("""UPDATE paper_trades SET qty_open=?,realized_pnl=?,exit_fees=exit_fees+?,
        status=?,closed_at=?,exit_reason=?,exit_price=?,r_multiple=? WHERE id=?""",
        (new_qty,realized,fee,status,int(time.time()) if status=="CLOSED" else None,
         reason if status=="CLOSED" else tr["exit_reason"],fill if status=="CLOSED" else tr["exit_price"],rmult,tr["id"]))
    con.execute("UPDATE paper_account SET cash=cash+?,realized_pnl=realized_pnl+?,fees_paid=fees_paid+?,updated_at=? WHERE id=1",
                (proceeds-fee,pnl,fee,int(time.time())))
    con.commit()

def paper_manage_trade(con,tr,price):
    if price<=0:return
    con.execute("UPDATE paper_trades SET max_price=?,min_price=? WHERE id=?",
                (max(sf(tr["max_price"]),price),min(sf(tr["min_price"],price),price),tr["id"]))
    con.commit()
    tr=con.execute("SELECT * FROM paper_trades WHERE id=?",(tr["id"],)).fetchone()
    stop,t1,t2=sf(tr["stop_current"]),sf(tr["target1"]),sf(tr["target2"])
    if price<=stop:
        paper_realize(con,tr,sf(tr["qty_open"]),stop,"STOP",True);return
    age=(int(time.time())-int(tr["opened_at"]))/3600
    if age>=PAPER_MAX_TRADE_AGE_HOURS:
        paper_realize(con,tr,sf(tr["qty_open"]),price,"TIME EXIT",True);return
    if not int(tr["t1_hit"]) and t1>0 and price>=t1:
        paper_realize(con,tr,sf(tr["qty_open"])*.5,t1,"T1 PARTIAL",False)
        con.execute("UPDATE paper_trades SET t1_hit=1,stop_current=? WHERE id=?",(sf(tr["entry_fill"]),tr["id"]))
        con.commit()
        tr=con.execute("SELECT * FROM paper_trades WHERE id=?",(tr["id"],)).fetchone()
    if t2>0 and price>=t2 and sf(tr["qty_open"])>0:
        paper_realize(con,tr,sf(tr["qty_open"]),t2,"TARGET2",True)

def paper_summary():
    try:
        con=paper_db()
        acc=con.execute("SELECT * FROM paper_account WHERE id=1").fetchone()
        closed=list(con.execute("SELECT * FROM paper_trades WHERE status='CLOSED' ORDER BY closed_at"))
        opened=paper_open_rows(con)
        pnls=[sf(r["realized_pnl"])-sf(r["entry_fee"]) for r in closed]
        wins=[x for x in pnls if x>0];losses=[x for x in pnls if x<0]
        gp=sum(wins);gl=abs(sum(losses))
        pf=gp/gl if gl>0 else (999.0 if gp>0 else None)
        rs=[sf(r["r_multiple"]) for r in closed if r["r_multiple"] is not None]
        eq,open_value=paper_equity_value(con)
        snaps=list(con.execute("SELECT equity FROM paper_equity ORDER BY ts"))
        peak=None;maxdd=0
        for s in snaps:
            e=sf(s["equity"]);peak=e if peak is None else max(peak,e)
            if peak:maxdd=max(maxdd,(peak-e)/peak)
        recent=[dict(r) for r in con.execute("""SELECT id,symbol,setup_type,opened_at,closed_at,status,
            entry_fill,stop_initial,target1,target2,realized_pnl,entry_fee,r_multiple,exit_reason
            FROM paper_trades ORDER BY id DESC LIMIT 12""")]
        con.close()
        return {
            "enabled":PAPER_ENABLED,"starting_balance":round(sf(acc["starting_balance"]),2),
            "cash":round(sf(acc["cash"]),2),"equity":round(eq,2),"open_value":round(open_value,2),
            "net_pnl":round(eq-sf(acc["starting_balance"]),2),"fees_paid":round(sf(acc["fees_paid"]),2),
            ...
            "max_drawdown_pct":round(maxdd*100,2),"recent":recent,
            "settings":{"risk_per_trade_pct":PAPER_RISK_PER_TRADE*100,"max_open_trades":PAPER_MAX_OPEN_TRADES,
                        "fee_pct":PAPER_FEE_RATE*100,"slippage_pct":PAPER_SLIPPAGE_RATE*100,
                        "interval_seconds":PAPER_INTERVAL_SECONDS}
        }
    except Exception as e:
        return {"enabled":PAPER_ENABLED,"error":str(e),"recent":[]}

def run_paper_cycle():
    if not PAPER_ENABLED:return {"enabled":False}
    con=paper_db()
    try:
        for tr in paper_open_rows(con):
            paper_manage_trade(con,tr,paper_quote(paper_signal_from_row(tr)))
        data=get_scan("crypto",PAPER_HORIZON,PAPER_RISK_MODE,False)
        rows=data.get("results") or []
        for s in rows:
            if len(paper_open_rows(con))>=PAPER_MAX_OPEN_TRADES:break
            sym=str(s.get("symbol") or "").upper()
            if not sym or con.execute("SELECT 1 FROM paper_trades WHERE status='OPEN' AND symbol=?",(sym,)).fetchone():continue
            if not paper_eligible(s):continue
            px=paper_quote(s)
            if px>0 and paper_in_entry_zone(px,s):
                paper_open_trade(con,s,px)
        eq,open_value=paper_equity_value(con)
        acc=con.execute("SELECT * FROM paper_account WHERE id=1").fetchone()
        con.execute("INSERT OR REPLACE INTO paper_equity(ts,cash,open_value,equity,open_trades) VALUES(?,?,?,?,?)",
                    (int(time.time()),sf(acc["cash"]),open_value,eq,len(paper_open_rows(con))))
        con.commit()
    finally:
        con.close()
    return paper_summary()

def paper_background_loop():
    while True:
        try:
            run_paper_cycle()
        except Exception as e:
...
def start_paper_thread_once():
    global paper_thread_started
    if PAPER_ENABLED and not paper_thread_started:
        paper_thread_started=True
        threading.Thread(target=paper_background_loop,name="paper-trader",daemon=True).start()
...
<style>
:root{
 --bg:#07111c;--panel:#0c1826;--panel2:#101e2d;--line:#21364d;--text:#f1f7ff;
 --muted:#8da2b9;--blue:#6fd6ff;--violet:#9588ff;--green:#6fe3ac;--yellow:#f9d77b;--red:#ff91a0;
}
*{box-sizing:border-box}html{scroll-behavior:smooth}
body{margin:0;background:radial-gradient(circle at 10% -10%,rgba(111,214,255,.16),transparent 30%),radial-gradient(circle at 90% 0%,rgba(149,136,255,.13),transparent 28%),var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,Inter,Segoe UI,Arial,sans-serif}
.shell{max-width:1120px;margin:auto;padding:0 15px 80px}
nav{height:68px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,.06);position:sticky;top:0;background:rgba(7,17,28,.88);backdrop-filter:blur(18px);z-index:20}
.logo{display:flex;align-items:center;gap:10px;font-weight:950;letter-spacing:-.03em}.mark{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,var(--blue),var(--violet));box-shadow:0 8px 25px rgba(111,214,255,.18);display:grid;place-items:center;color:#07111c;font-weight:1000}.navRight{display:flex;gap:7px}.navChip{font-size:10px;padding:7px 9px;border:1px solid var(--line);border-radius:999px;color:#9db0c5;background:#0c1722}.navChip.live{color:var(--green)}
.hero{padding:30px 0 18px}.eyebrow{font-size:10px;text-transform:uppercase;letter-spacing:.16em;color:var(--blue);font-weight:900}.hero h1{font-size:clamp(30px,6vw,48px);letter-spacing:-.055em;line-height:1;margin:8px 0 10px}.hero p{max-width:680px;color:#9fb0c3;font-size:14px;line-height:1.6;margin:0}
.pulse{display:grid;grid-template-columns:1.2fr repeat(4,1fr);gap:8px;margin:18px 0}.pulseMain,.pulseBox{background:linear-gradient(180deg,#101f30,#0b1723);border:1px solid var(--line);border-radius:16px;padding:13px}.pulseMain small,.pulseBox small{display:block;color:#768ba2;text-transform:uppercase;letter-spacing:.08em;font-size:9px}.pulseMain b{display:block;font-size:20px;margin-top:4px}.pulseBox b{display:block;font-size:14px;margin-top:5px}
.controls{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;padding:11px;background:#0a1521;border:1px solid var(--line);border-radius:16px;margin:12px 0 15px;position:sticky;top:78px;z-index:12;box-shadow:0 16px 40px rgba(0,0,0,.2)}
select,button{border:1px solid #294057;background:#111f2e;color:var(--text);border-radius:11px;padding:11px;font-size:13px}button{font-weight:900;cursor:pointer}#scan{background:linear-gradient(135deg,#eff8ff,#cfeaff);color:#07111c;border:0;min-width:155px}
.infoRow{display:flex;gap:7px;flex-wrap:wrap;margin:8px 0}.info{font-size:10px;color:#8fa3ba;padding:7px 9px;border:1px solid var(--line);border-radius:999px;background:#0a1520}
.warn{border:1px solid #633740;background:#2a171d;color:#ffc2ca;border-radius:13px;padding:11px;margin:10px 0;font-size:12px}.diag{font-size:10px;color:#71869d;margin:8px 2px}
.sectionTitle{display:flex;align-items:end;justify-content:space-between;margin:23px 2px 10px}.sectionTitle h2{font-size:18px;margin:0}.sectionTitle span{font-size:10px;color:#6f849b}
.focusWrap{display:none}.focusWrap.show{display:block}.focus,.card{border:1px solid #1f344a;background:linear-gradient(180deg,#0f1d2b,#091622);border-radius:18px;box-shadow:0 14px 35px rgba(0,0,0,.14)}.focus{padding:16px;border-color:#3a5a79}
.cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.card{padding:14px;position:relative;overflow:hidden}.card:before{content:"";position:absolute;inset:0 0 auto;height:2px;background:linear-gradient(90deg,transparent,rgba(111,214,255,.6),transparent);opacity:.45}
.head{display:flex;justify-content:space-between;gap:12px}.rank{font-size:9px;color:#6f859d;text-transform:uppercase;letter-spacing:.1em}.name{font-size:20px;font-weight:950;margin-top:3px}.sym{font-size:10px;color:#72869d;text-transform:uppercase}.score{font-size:25px;font-weight:950;text-align:right}.score small{display:block;font-size:8px;color:#6c8097;letter-spacing:.09em}
.price{font-size:18px;font-weight:900;margin:10px 0}.delta{font-size:10px;margin-left:5px}.liveState{font-size:9px;color:var(--green);margin-left:6px}.dot{display:inline-block;width:6px;height:6px;background:var(--green);border-radius:50%;margin-right:4px}
.tags{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}.tag{font-size:9px;padding:6px 8px;border-radius:999px;border:1px solid #2a4057;background:#101e2c;color:#aebed0}.good{background:#10261c;border-color:#285c45;color:#82e8b7}.bad{background:#2a171c;border-color:#63363d;color:#ffb0bb}.mid{background:#2a2518;border-color:#63552f;color:#ffe39a}.cb{font-weight:950}
.actionLine{display:flex;align-items:center;justify-content:space-between;margin:10px 0 4px}.action{font-size:11px;font-weight:950;letter-spacing:.07em}.ready{color:var(--green)}.wait{color:var(--yellow)}.avoid{color:var(--red)}.progress{font-size:9px;color:#7388a0}
.bar{height:5px;background:#101c29;border-radius:999px;overflow:hidden;margin:5px 0 10px}.bar>span{display:block;height:100%;background:linear-gradient(90deg,var(--blue),var(--violet));border-radius:999px}
.plan{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}.planBox,.metric{background:#0a1520;border:1px solid #1d3044;border-radius:10px;padding:8px;color:#778ba3;font-size:9px}.planBox b,.metric b{display:block;color:#eef5fd;font-size:12px;margin-top:3px}
.callout,.reason{margin-top:9px;padding:10px;border-radius:11px;font-size:10px;line-height:1.5}.callout{background:#101c29;border:1px solid #24384e;color:#b6c5d5}.reason{background:#08131d;border:1px solid #1a2b3e;color:#aebfd1}
.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:9px}.actions{display:flex;gap:7px;margin-top:10px}.actions button{padding:9px}.focusBtn{background:#dceeff;color:#07111c;border:0}
details{margin-top:10px;padding-top:9px;border-top:1px solid #1b3044}summary{cursor:pointer;color:#9fb2c8;font-size:10px;font-weight:900}.advanced{padding-top:5px}.news{margin-top:9px}.news div{font-size:10px;color:#aab9cb;line-height:1.45;margin:4px 0}
.focus .name{font-size:26px}.focus .price{font-size:30px}.empty{text-align:center;color:#7d91a8;padding:46px 12px}.footer{margin-top:30px;border-top:1px solid #172b3e;padding:15px 2px;color:#60758c;font-size:9px;line-height:1.55}
@media(max-width:820px){.cards{grid-template-columns:1fr}.pulse{grid-template-columns:1fr 1fr}.pulseMain{grid-column:1/-1}.controls{grid-template-columns:1fr 1fr}.controls select:first-child{grid-column:1/-1}#scan{grid-column:1/-1}.plan,.grid{grid-template-columns:repeat(2,1fr)}}

.evidenceBox{margin-top:9px;padding:10px;border-radius:11px;background:#0b1723;border:1px solid #20354a;color:#9fb1c5;font-size:10px}
.evidenceBox b{color:#eef6ff}.noTradeBanner{margin:12px 0;padding:13px;border-radius:14px;background:#2a171d;border:1px solid #653741;color:#ffc2cb;font-size:12px;font-weight:850}
.confluence{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:9px}


.paperPanel{margin:24px 0 8px;padding:15px;border-radius:18px;border:1px solid #21374d;background:linear-gradient(180deg,#0f1d2b,#091620)}
.paperHead{display:flex;justify-content:space-between;align-items:center;gap:12px}.paperHead h2{margin:0;font-size:18px}.paperHead span{font-size:10px;color:#7890a7}
.paperGrid{display:grid;grid-template-columns:repeat(6,1fr);gap:7px;margin-top:12px}.paperStat{background:#091520;border:1px solid #1c3044;border-radius:11px;padding:10px}.paperStat small{display:block;color:#748aa1;font-size:9px;text-transform:uppercase}.paperStat b{display:block;font-size:15px;margin-top:4px}
.paperTrades{margin-top:12px;overflow:auto}.paperTable{width:100%;border-collapse:collapse;font-size:10px}.paperTable th,.paperTable td{padding:8px;border-bottom:1px solid #192c3e;text-align:left;white-space:nowrap}.paperTable th{color:#71869d;font-weight:800}.paperGood{color:#6fe3ac}.paperBad{color:#ff91a0}
@media(max-width:820px){.paperGrid{grid-template-columns:repeat(2,1fr)}}
</style>
</head>
<body><div class="shell">
<nav><div class="logo"><div class="mark">R</div><span>Crypto Radar</span></div><div class="navRight"><span class="navChip live">● Live market data</span><span class="navChip">Manual rankings</span></div></nav>

<section class="hero"><div class="eyebrow">Market intelligence · day trading</div><h1>CryptO Radar</h1></section>

<div class="pulse">
 <div class="pulseMain"><small>Market regime</small><b id="marketRegime">Loading…</b></div>
 <div class="pulseBox"><small>Alt environment</small><b id="altEnv">—</b></div>
 <div class="pulseBox"><small>Breadth</small><b id="breadth">—</b></div>
 <div class="pulseBox"><small>Fear & Greed</small><b id="fear">—</b></div>
 <div class="pulseBox"><small>Last scan</small><b id="updated">—</b></div>
</div>
...
<div class="controls">
 ...
 <select id="horizon"><option value="1h">1 Hour · Scalping</option><option value="4h" selected>4 Hours · Day Trade</option><option value="12h">12 Hours</option><option value="24h">24 Hours</option><option value="3d">3 Days</option><option value="1w">1 Week</option></select>
...
<div class="infoRow"><span class="info" id="provider">Data —</span><span class="info" id="scanStats">Waiting for scan</span><span class="info">Rankings stay fixed while you read</span></div>
<div id="warning"></div><div id="diagnostics"></div>

<div id="focusWrap" class="focusWrap"><div class="sectionTitle"><h2>Focus Mode</h2><span>Live price, stable analysis</span></div><div id="focusCard" class="focus"></div></div>
<div class="sectionTitle"><h2>Top Opportunities</h2><span>Ranked by setup quality, not hype</span></div>
<div id="results" class="cards"><div class="empty">Starting radar…</div></div>

<section class="paperPanel"><div class="paperHead"><div><h2>Paper Trading</h2><span>Simulated trades using Radar signals</span></div><span id="paperStatus">Loading…</span></div><div class="paperGrid"><div class="paperStat"><small>Paper equity</small><b id="paperEquity">—</b></div><div class="paperStat"><small>Net P&L</small><b id="paperPnl">—</b></div><div class="paperStat"><small>Open trades</small><b id="paperOpen">—</b></div><div class="paperStat"><small>Win rate</small><b id="paperWin">—</b></div><div class="paperStat"><small>Profit factor</small><b id="paperPf">—</b></div><div class="paperStat"><small>Max drawdown</small><b id="paperDd">—</b></div></div><div class="paperTrades" id="paperTrades"></div></section>
<div class="footer">Research and decision-support only. Scores and trade plans are not guarantees or calibrated probabilities unless enough paper-tracked samples exist. Trading crypto can result in substantial losses; stops and targets can be skipped during fast markets.</div>
</div>
<script>
const money=x=>{if(x===null||x===undefined||!Number.isFinite(Number(x)))return'—';x=Number(x);if(x>=1e9)return'$'+(x/1e9).toFixed(2)+'B';if(x>=1e6)return'$'+(x/1e6).toFixed(2)+'M';if(x>=1000)return'$'+x.toLocaleString(undefined,{maximumFractionDigits:2});if(x>=1)return'$'+x.toFixed(4);if(x>=.01)return'$'+x.toFixed(5);return'$'+x.toFixed(8)}
const pct=x=>(Number(x)>=0?'+':'')+Number(x||0).toFixed(2)+'%';
const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
const state=c=>{if(c.action==='AVOID')return['AVOID','bad'];if(c.action==='SETUP READY')return['SETUP READY','good'];if((c.prebreakout_score??50)>=70)return['EARLY SETUP','good'];return['WAIT / WATCH','mid']}
const coinbase=c=>c.coinbase_available===true?'<span class="tag good cb">Coinbase ✓</span>':c.coinbase_available===false?'<span class="tag bad cb">Coinbase ✕</span>':'<span class="tag mid cb">Coinbase ?</span>';
let data=null,focused=null,focusSnapshot=null,socket=null,reconnect=null,liveMap={};
function stopLive(){if(reconnect){clearTimeout(reconnect);reconnect=null}if(socket){try{socket.onclose=null;socket.close()}catch(_){}socket=null}liveMap={}}
function startLive(rows){stopLive();const list=(rows||[]).map((c,i)=>({c,i})).filter(x=>x.c.live_pair).slice(0,20);if(!list.length)return;const streams=[];list.forEach(({c,i})=>{const p=c.live_pair.toUpperCase();liveMap[p]={c,i};streams.push(p.toLowerCase()+'@ticker');streams.push(p.toLowerCase()+'@bookTicker')});try{socket=new WebSocket('wss://stream.binance.us:9443/stream?streams='+streams.join('/'))}catch(_){return}socket.onopen=()=>list.forEach(({i})=>{const e=document.getElementById('live-'+i);if(e)e.innerHTML='<span class="dot"></span>LIVE'});socket.onmessage=ev=>{let m;try{m=JSON.parse(ev.data)}catch(_){return};const d=m.data||m,x=liveMap[d.s];if(!x)return;if(d.c!==undefined){const p=Number(d.c),e=document.getElementById('price-'+x.i);if(p>0&&e)e.textContent=money(p);if(focused===String(x.c.symbol).toUpperCase()){const f=document.getElementById('focusPrice');if(f&&p>0)f.textContent=money(p)}}};socket.onclose=()=>reconnect=setTimeout(()=>startLive(data?.results||[]),5000)}
function lifecycle(c){return c.setup_lifecycle||{stage:'WATCHING',progress:25,next:'wait for confirmation'}}
...
function renderFocus(c){if(!c)return;const[s,cls]=state(c),lc=lifecycle(c),lm=c.liquidity_map||{},der=c.derivatives||{};document.getElementById('focusCard').innerHTML=`<div class="head"><div><div class="name">${esc(c.name)}</div><div class="sym">${esc(c.symbol)} · ${esc(c.setup_type||'Setup')}</div></div><button style="width:auto" onclick="closeFocus()">Close</button></div><div id="focusPrice" class="price">${money(c.price)}</div><div class="tags"><span class="tag ${cls}">${s}</span>${coinbase(c)}<span class="tag">Market ${esc(c.market_alignment||'—')}</span><span class="tag">Flow ${esc(c.orderbook_pressure||'—')}</span><span class="tag">Toxicity ${esc(c.toxicity_label||'—')}</span></div><div class="actionLine"><span class="action ${c.action==='SETUP READY'?'ready':c.action==='AVOID'?'avoid':'wait'}">${esc(lc.stage)}</span><span class="progress">${lc.progress??0}% through setup</span></div><div class="bar"><span style="width:${Math.max(0,Math.min(100,lc.progress??0))}%"></span></div><div class="plan"><div class="planBox">Entry zone<b>${money(c.entry_low)} – ${money(c.entry_high)}</b></div><div class="planBox">Invalidation<b>${money(c.stop)}</b></div><div class="planBox">Targets<b>${money(c.target1)} / ${money(c.target2)}</b></div><div class="planBox">Risk / reward<b>${c.risk_reward??'—'} : 1</b></div></div><div class="callout"><b>Next confirmation:</b> ${esc(lc.next||c.trigger||'Wait for confirmation')}</div><div class="grid"><div class="metric">Confirmation<b>${c.confirmation_quality??'—'}</b></div><div class="metric">Microstructure<b>${c.microstructure_score??'—'}</b></div><div class="metric">Pre-breakout<b>${c.prebreakout_score??'—'}</b></div><div class="metric">No-trade risk<b>${c.do_not_trade_score??'—'}</b></div><div class="metric">Liquidity above<b>${lm.nearest_above?lm.nearest_above.distance_pct+'%':'—'}</b></div><div class="metric">Liquidity below<b>${lm.nearest_below?lm.nearest_below.distance_pct+'%':'—'}</b></div><div class="metric">Funding<b>${der.funding_rate!==null&&der.funding_rate!==undefined?der.funding_rate+'%':'—'}</b></div><div class="metric">OI change<b>${der.oi_change_pct!==null&&der.oi_change_pct!==undefined?der.oi_change_pct+'%':'—'}</b></div></div><div class="reason"><b>Why it matters</b><br>${(c.decision_summary?.best_reasons||c.prebreakout_reasons||[]).map(x=>'✓ '+esc(x)).join('<br>')||'No strong confirmation yet.'}${(c.decision_summary?.main_risks||[]).length?'<br><br>'+c.decision_summary.main_risks.map(x=>'⚠ '+esc(x)).join('<br>'):''}</div><div class="reason"><b>Execution style:</b> ${esc(c.execution_style||'balanced')} · <b>Risk budget:</b> ${esc(c.risk_budget_hint||'—')}</div>`}
function card(c,i){const[s,cls]=state(c),lc=lifecycle(c),lm=c.liquidity_map||{},der=c.derivatives||{},deltaColor=Number(c.change_24h)>=0?'#6fe3ac':'#ff91a0';return`<article class="card"><div class="head"><div><div class="rank">#${i+1} · ${esc(c.setup_type||c.scenario)}</div><div class="name">${esc(c.name)}</div><div class="sym">${esc(c.symbol)}</div></div><div class="score">${c.score}<small>RADAR</small></div></div><div class="price"><span id="price-${i}">${money(c.price)}</span><span class="delta" style="color:${deltaColor}">${pct(c.change_24h)}</span>${c.live_pair?`<span id="live-${i}" class="liveState"><span class="dot"></span>connecting</span>`:''}</div><div class="tags"><span class="tag ${cls}">${s}</span>${coinbase(c)}<span class="tag">Stage ${esc(lc.stage)}</span><span class="tag">Flow ${esc(c.orderbook_pressure||'—')}</span><span class="tag">Derivatives ${esc(der.label||'—')}</span></div><div class="actionLine"><span class="action ${c.action==='SETUP READY'?'ready':c.action==='AVOID'?'avoid':'wait'}">${esc(c.action||'WATCH')}</span><span class="progress">${lc.progress??0}% setup progress</span></div><div class="bar"><span style="width:${Math.max(0,Math.min(100,lc.progress??0))}%"></span></div><div class="plan"><div class="planBox">Entry<b>${money(c.entry_low)} – ${money(c.entry_high)}</b></div><div class="planBox">Stop<b>${money(c.stop)}</b></div><div class="planBox">Targets<b>${money(c.target1)} / ${money(c.target2)}</b></div><div class="planBox">R:R<b>${c.risk_reward??'—'} : 1</b></div></div><div class="callout"><b>Wait for:</b> ${esc(lc.next||c.trigger||'confirmation')}</div><div class="grid"><div class="metric">Confirm<b>${c.confirmation_quality??'—'}</b></div><div class="metric">Microstructure<b>${c.microstructure_score??'—'}</b></div><div class="metric">Entry quality<b>${c.entry_quality??'—'}</b></div><div class="metric">Pre-breakout<b>${c.prebreakout_score??'—'}</b></div><div class="metric">Toxicity<b>${c.toxicity_score??'—'}</b></div><div class="metric">Conflict<b>${c.signal_disagreement??'—'}</b></div><div class="metric">Liquidity ↑<b>${lm.nearest_above?lm.nearest_above.distance_pct+'%':'—'}</b></div><div class="metric">Liquidity ↓<b>${lm.nearest_below?lm.nearest_below.distance_pct+'%':'—'}</b></div></div><div class="confluence"><div class="metric">Technical family<b>${c.confluence_groups?.technical??'—'}</b></div><div class="metric">Microstructure family<b>${c.confluence_groups?.microstructure??'—'}</b></div><div class="metric">Market context<b>${c.confluence_groups?.market_context??'—'}</b></div><div class="metric">Info / derivatives<b>${c.confluence_groups?.information??'—'}</b></div></div><div class="evidenceBox"><b>Historical evidence:</b> ${c.evidence?.calibrated?`${c.evidence.win_rate}% wins across ${c.evidence.samples} comparable paper-tracked setups`:`Collecting evidence · ${c.evidence?.samples??0} resolved samples so far`}</div>${(c.quality_gate_reasons||[]).length?`<div class="reason"><b>Why this is not entry-ready</b><br>${c.quality_gate_reasons.map(x=>'• '+esc(x)).join('<br>')}</div>`:''}<div class="actions"><button class="focusBtn" onclick="focusIndex(${i})">OPEN FOCUS MODE</button></div><details><summary>Deep analysis</summary><div class="advanced"><div class="reason"><b>Market structure</b><br>${esc(c.structure||'—')} · support ${money(c.support)} · resistance ${money(c.resistance)} · absorption ${esc(c.absorption?.label||'—')}</div><div class="reason"><b>Derivatives</b><br>${esc(der.label||'unavailable')} · funding ${der.funding_rate??'—'}% · OI ${der.oi_change_pct??'—'}% · basis ${der.basis_pct??'—'}%</div><div class="reason"><b>News intelligence</b><br>${esc(c.news_intelligence?.event_type||'—')} · credibility ${c.news_intelligence?.credibility_score??'—'} · novelty ${c.news_intelligence?.novelty_score??'—'}</div>${(c.chart_signals||[]).length?`<div class="reason"><b>Price-action signals</b><br>${c.chart_signals.slice(0,10).map(x=>'• '+esc(x)).join('<br>')}</div>`:''}${(c.headlines||[]).length?`<div class="news">${c.headlines.slice(0,5).map(h=>`<div>• ${esc(h.title)}</div>`).join('')}</div>`:''}</div></details></article>`}
function render(d){data=d;document.getElementById('marketRegime').textContent=(d.regime?.label||'—')+' · '+(d.regime?.score??'—')+'/100';document.getElementById('altEnv').textContent=(d.regime?.alt_environment||'—')+' · '+(d.regime?.alt_score??'—');document.getElementById('breadth').textContent=(d.regime?.breadth??'—')+'/100';document.getElementById('fear').textContent=(d.fear_greed?.value??'—')+' · '+(d.fear_greed?.label||'—');document.getElementById('updated').textContent=new Date(d.updated*1000).toLocaleTimeString();document.getElementById('provider').textContent='Data: '+(d.provider||'—');const dg=d.diagnostics||{};document.getElementById('scanStats').textContent=`Technicals ${dg.technical_completed??0}/${dg.technical_requested??0} · News ${dg.news_completed??0}/${dg.news_requested??0} · Derivatives ${dg.derivatives_completed??0}/${dg.derivatives_requested??0}`;document.getElementById('warning').innerHTML=(d.warning?'<div class="warn">'+esc(d.warning)+'</div>':'')+(d.no_trade_market?'<div class="noTradeBanner">NO TRADE: no setup passed the current quality gate.</div>':'');document.getElementById('diagnostics').innerHTML=dg.hard_timeout?'<div class="diag">The server returned partial data at its deadline instead of hanging.</div>':'';const rows=d.results||[];document.getElementById('results').innerHTML=rows.length?rows.map(card).join(''):'<div class="empty">No ranked setups returned. Try Refresh Rankings.</div>';startLive(rows);if(focused){const f=rows.find(x=>String(x.symbol).toUpperCase()===focused);renderFocus(f||focusSnapshot)}}
async function scan(force=false){const b=document.getElementById('scan');b.disabled=true;b.textContent='SCANNING…';if(!data)document.getElementById('results').innerHTML='<div class="empty">Reading market structure, liquidity, order flow and news…</div>';try{const a=document.getElementById('asset').value,h=document.getElementById('horizon').value,r=document.getElementById('risk').value,ctl=new AbortController(),kill=setTimeout(()=>ctl.abort(),33000);let res;try{res=await fetch(`/api/scan?asset=${a}&horizon=${h}&risk=${r}&force=${force?1:0}`,{signal:ctl.signal})}finally{clearTimeout(kill)}const d=await res.json();if(d.error)throw new Error(d.error);render(d)}catch(e){document.getElementById('warning').innerHTML='<div class="warn">'+esc(e.name==='AbortError'?'The scan hit its safety deadline. Tap Refresh Rankings to retry; completed cached data is preserved.':e.message)+'</div>';if(!data)document.getElementById('results').innerHTML='<div class="empty">Scan stopped instead of hanging. Tap Refresh Rankings.</div>'}b.disabled=false;b.textContent='REFRESH RANKINGS'}

async function loadPaper(){
 try{
  const r=await fetch('/api/paper/summary',{cache:'no-store'}),p=await r.json();
  const pnl=Number(p.net_pnl||0);
  document.getElementById('paperStatus').textContent=p.enabled?'Paper engine active':'Paper engine disabled';
  document.getElementById('paperEquity').textContent=money(p.equity);
  document.getElementById('paperPnl').textContent=(pnl>=0?'+':'')+money(pnl);
  document.getElementById('paperPnl').className=pnl>=0?'paperGood':'paperBad';
  document.getElementById('paperOpen').textContent=p.open_trades??'—';
  document.getElementById('paperWin').textContent=p.win_rate===null||p.win_rate===undefined?'Collecting':p.win_rate+'%';
  document.getElementById('paperPf').textContent=p.profit_factor??'—';
  document.getElementById('paperDd').textContent=(p.max_drawdown_pct??0)+'%';
  const rows=p.recent||[];
  document.getElementById('paperTrades').innerHTML=rows.length?`<table class="paperTable"><thead><tr><th>Coin</th><th>Setup</th><th>Status</th><th>Entry</th><th>T1</th><th>T2</th><th>Result</th></tr></thead><tbody>${rows.map(x=>{const rp=Number(x.realized_pnl||0)-Number(x.entry_fee||0);return`<tr><td>${esc(x.symbol)}</td><td>${esc(x.setup_type||'—')}</td><td>${esc(x.status)}</td><td>${money(x.entry_fill)}</td><td>${money(x.target1)}</td><td>${money(x.target2)}</td><td class="${rp>=0?'paperGood':'paperBad'}">${x.status==='CLOSED'?(rp>=0?'+':'')+money(rp):'OPEN'}</td></tr>`}).join('')}</tbody></table>`:'<div class="reason">No paper trades yet. The engine is waiting for a setup that passes the quality gate and enters its planned entry zone.</div>';
 }catch(e){
  document.getElementById('paperStatus').textContent='Paper data unavailable';
 }
}

document.getElementById('scan').onclick=()=>scan(false);['asset','horizon','risk'].forEach(id=>document.getElementById(id).onchange=()=>scan(false));document.addEventListener('visibilitychange',()=>{if(document.hidden)stopLive();else if(data)startLive(data.results||[])});scan(false);loadPaper();setInterval(loadPaper,15000);
</script></body></html>"""
...
def api_health():
    return jsonify({
        ...
        "features":["coinbase-status","focus-mode","trade-plan","do-not-trade","setup-validation","decision-quality","ttl-cache","single-flight"],"cache":cache_stats()
    })

@app.route("/api/calibration")
def api_calibration():
    return jsonify({"all":performance_breakdown(),"minimum_samples":MIN_CALIBRATION_SAMPLES,
                    "note":"Historical paper-tracked outcomes only; not a guarantee of future performance."})
...
@app.route("/version")
def version():
    return {"version":"17.0-microstructure","auto_ranking_refresh":False}


@app.route("/api/paper/summary")
def api_paper_summary():
    return jsonify(paper_summary())

@app.route("/api/paper/cycle", methods=["POST"])
def api_paper_cycle():
    return jsonify(run_paper_cycle())

start_paper_thread_once()
