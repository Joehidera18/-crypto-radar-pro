...
# ============================================================
# CRYPTO RADAR PRO v11 — DAY-TRADING RADAR + FOCUS MODE
# ============================================================
#
# Goals:
...
<title>Crypto Radar Pro V11</title>
<style>
...
<body>
<div class="wrap">
<div class="brand"><span class="chip v11">V11</span><span class="chip live">● LIVE PRICES</span><span class="chip locked">🔒 MANUAL RANKINGS</span></div>
<h1>Crypto Radar Pro</h1>
<div class="subtitle">Day-trading radar with stable cards, live prices, Coinbase availability, news and social context.</div>
<div class="notice"><b>No automatic reshuffling in V11.</b> Prices stay live, but the ranking only changes when you press <b>Refresh Rankings</b>. Tap <b>Focus</b> and the coin stays open while you read.</div>
...
<div class="footer">Research tool only. Rankings are not guaranteed returns or calibrated probabilities. Coinbase status is based on active Coinbase Exchange USD/USDC/USDT products and may differ by jurisdiction/account.</div>
</div>
...
function renderFocus(c){if(!c)return;const[s,cls]=state(c),tf=c.timeframes||{},tfText=['1m','5m','10m','15m','1h','4h'].filter(k=>tf[k]).map(k=>`${k}: ${tf[k].score}/100 · ${tf[k].trend}`).join('<br>');document.getElementById('focusCard').innerHTML=`<div class="head"><div><div class="name">${esc(c.name)}</div><div class="sym">${esc(c.symbol)} · ${esc(c.market_phase||'—')}</div></div><button style="width:auto" onclick="closeFocus()">Close</button></div><div id="focusPrice" class="price" style="font-size:28px">${money(c.price)}</div><div class="tags"><span class="tag ${cls}">${s}</span>${coinbase(c)}<span class="tag">Trend ${esc(c.trend||'—')}</span></div><div class="grid"><div class="metric">Radar<b>${c.score}</b></div><div class="metric">Entry<b>${c.entry_quality}</b></div><div class="metric">Pre-breakout<b>${c.prebreakout_score}</b></div><div class="metric">Technical<b>${c.technical_score}</b></div><div class="metric">News<b>${c.news_score}</b></div><div class="metric">Social<b>${c.social_score??'—'}</b></div></div><div class="reason"><b>What the radar sees</b><br>${(c.prebreakout_reasons||c.why_now||[]).slice(0,6).map(x=>'• '+esc(x)).join('<br>')||'No strong explanation available.'}</div>${tfText?`<div class="reason"><b>Short-timeframe alignment</b><br>${tfText}</div>`:''}`}
...
function card(c,i){const[s,cls]=state(c),reasons=(c.prebreakout_reasons||c.why_now||[]).slice(0,3);return `<div class="card"><div class="head"><div><div class="rank">#${i+1} · ${esc(c.scenario)}</div><div class="name">${esc(c.name)}</div><div class="sym">${esc(c.symbol)}</div></div><div class="score">${c.score}<small>RADAR SCORE</small></div></div><div class="price"><span id="price-${i}">${money(c.price)}</span> <span style="font-size:11px;color:${Number(c.change_24h)>=0?'#6ce6aa':'#ff8b99'}">${pct(c.change_24h)} 24h</span>${c.live_pair?`<span id="live-${i}" class="live-state"><span class="dot"></span>connecting</span>`:''}</div><div class="tags"><span class="tag ${cls}">${s}</span>${coinbase(c)}<span class="tag">Phase ${esc(c.market_phase||'—')}</span>${c.live_pair?`<span id="spread-${i}" class="tag">Spread —</span>`:''}</div><div class="grid"><div class="metric">Entry<b>${c.entry_quality}</b></div><div class="metric">Pre-breakout<b>${c.prebreakout_score}</b></div><div class="metric">Momentum<b>${c.momentum_score}</b></div><div class="metric">Technical<b>${c.technical_score}</b></div><div class="metric">News<b>${c.news_score}</b></div><div class="metric">Chase<b>${c.chase_penalty}</b></div></div><div class="callout ${cls}">${reasons.length?reasons.map(x=>'• '+esc(x)).join('<br>'):'No unusually strong early signal yet.'}</div><div class="actions"><button class="focusBtn" onclick="focusIndex(${i})">◎ FOCUS</button></div><details><summary>Advanced Analysis</summary><div class="reason"><b>Structure</b><br>${esc(c.structure||'—')}<br>Support ${money(c.support)} · Resistance ${money(c.resistance)}<br>${(c.chart_signals||[]).slice(0,8).map(x=>'• '+esc(x)).join('<br>')}</div>${(c.headlines||[]).length?`<div class="news">${(c.headlines||[]).slice(0,5).map(h=>`<div>• ${esc(h.title)}</div>`).join('')}</div>`:''}${(c.reddit_posts||[]).length?`<div class="news">${(c.reddit_posts||[]).slice(0,4).map(h=>`<div>• ${esc(h.title)}</div>`).join('')}</div>`:''}</details></div>`}
...
// V11 intentionally has NO setInterval scan. Rankings never change unless you request it.
</script>
</body>
</html>"""
...
@app.route("/version")
def version():
    return {"version":"11.0-stable-ui","auto_ranking_refresh":False}
