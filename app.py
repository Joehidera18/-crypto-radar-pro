...
def scan_crypto(horizon, risk):
    ...
    for a in prelim:
        ...
        rs_weight={"24h":0.10,"3d":0.10,"1w":0.09,"1m":0.07,"3m":0.05,"1y":0.03}.get(horizon,0.07)
        ...
        pre_w = {"24h":0.30,"3d":0.25,"1w":0.18,"1m":0.10,"3m":0.06,"1y":0.03}.get(horizon,0.15)
        ...
        accel_w={"24h":0.16,"3d":0.11,"1w":0.06,"1m":0.02,"3m":0.00,"1y":0.00}.get(horizon,0.05)
...
<div class="sub">
Top 20 crypto opportunities ranked using live market action, liquidity, technical structure, catalysts, news, risk and the broader crypto regime.
</div>
...
function startLiveStreams(results){
  ...
  eligible.forEach(({c,i})=>{
    ...
    liveIndexBySymbol[c.live_pair]={index:i,last:c.price,bid:null,ask:null};
...
// Refresh the model roughly every 90 seconds while this page is open.
// News is cached separately, so this does not repeatedly hit news feeds.
setInterval(()=>scan(true), 90000);
