...
# ============================================================
# CRYPTO RADAR PRO v15.2 — DAY-TRADING RADAR + FOCUS MODE
# ============================================================
#
# Goals:
...
def scan_crypto(horizon, risk):
    ...
    catalyst_map, news_done, news_pending = run_stage_with_deadline(
        news_assets,
        lambda a: catalyst_for_asset(a, horizon),
        "id",
        min(NEWS_STAGE_SECONDS, remaining),
        max_workers=8
    )
...
<title>Crypto Radar Pro V15.2</title>
<style>
...
<body>
<div class="wrap">
<div class="brand"><span class="chip v11">V15.2</span><span class="chip live">● LIVE PRICES</span><span class="chip locked">🔒 MANUAL RANKINGS</span><span class="chip">⚡ JS FIX + HARD STOP</span></div>
<h1>Crypto Radar Pro</h1>
<div class="subtitle">Day-trading radar with stable rankings, live prices and a shared data cache so technicals, history, Coinbase, news and social context are reused instead of needlessly downloaded again.</div>
<div class="notice"><b>No automatic reshuffling in V15.2.</b> Prices stay live, but the ranking only changes when you press <b>Refresh Rankings</b>. Tap <b>Focus</b> and the coin stays open while you read.</div>
...
<div class="section">Radar Rankings</div><div id="results" class="cards"><div class="empty">Starting radar…</div></div>
<div class="footer">Research tool only. Rankings are not guaranteed returns or calibrated probabilities. Coinbase status is based on active Coinbase Exchange USD/USDC/USDT products and may differ by jurisdiction/account. V15.2 paper-logs setups and begins showing historical setup win rate after at least 20 resolved samples. On Render's default ephemeral filesystem that history resets when the instance is replaced unless PREDICTION_DB_PATH points to persistent storage.</div>
</div>
...
function render(d){data=d;document.getElementById('updated').textContent='Updated '+new Date(d.updated*1000).toLocaleTimeString();document.getElementById('provider').textContent='Data '+(d.provider||'—');document.getElementById('regime').textContent='Market '+(d.regime?.label||'—');document.getElementById('fng').textContent='Fear & Greed '+(d.fear_greed?.value??'—')+' · '+(d.fear_greed?.label||'—');let w=[];if(d.warning)w.push(d.warning);document.getElementById('warning').innerHTML=w.map(x=>'<div class="warn">'+esc(x)+'</div>').join('');if(d.diagnostics)document.getElementById('diagnostics').innerHTML=`<div class="diag">Universe ${d.diagnostics.raw_universe||0} · candidates ${d.diagnostics.prelim_candidates||0} · technical ${d.diagnostics.technical_completed??d.diagnostics.technical_scanned??0}/${d.diagnostics.technical_requested??d.diagnostics.technical_scanned??0} · news/social ${d.diagnostics.news_completed??d.diagnostics.news_scanned??0}/${d.diagnostics.news_requested??d.diagnostics.news_scanned??0}</div>`;const r=d.results||[];document.getElementById('results').innerHTML=r.length?r.map(card).join(''):'<div class="empty">No coins passed these filters.</div>';startLive(r);if(focused){const fresh=r.find(x=>String(x.symbol).toUpperCase()===focused);renderFocus(fresh||focusSnapshot)}}
async function scan(force=true){const b=document.getElementById('scan');b.disabled=true;b.textContent='SCANNING…';if(!data)document.getElementById('results').innerHTML='<div class="empty">Requesting market data…</div>';document.getElementById('diagnostics').innerHTML='<div class="diag">Scan request started…</div>';try{const a=document.getElementById('asset').value,h=document.getElementById('horizon').value,r=document.getElementById('risk').value,ctl=new AbortController(),kill=setTimeout(()=>ctl.abort(),33000);let res;try{res=await fetch(`/api/scan?asset=${a}&horizon=${h}&risk=${r}&force=${force?1:0}`,{signal:ctl.signal})}finally{clearTimeout(kill)}const d=await res.json();if(d.error)throw new Error(d.error);render(d)}catch(e){document.getElementById('warning').innerHTML='<div class="warn">'+esc(e.name==='AbortError'?'Initial scan exceeded 33 seconds. Cached data is preserved; tap Refresh Rankings to retry.':e.message)+'</div>';if(!data)document.getElementById('results').innerHTML='<div class="empty">Scan stopped instead of hanging. Tap Refresh Rankings to retry.</div>'}b.disabled=false;b.textContent='REFRESH RANKINGS'}
document.getElementById('scan').onclick=()=>scan(false);['asset','horizon','risk'].forEach(id=>document.getElementById(id).onchange=()=>scan(true));document.addEventListener('visibilitychange',()=>{if(document.hidden)stopLive();else if(data)startLive(data.results||[])});scan(false);
...
@app.route("/version")
def version():
    return {"version":"15.2-js-fixed","auto_ranking_refresh":False}
